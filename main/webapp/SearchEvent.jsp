...
<div class="container content">
    ...
    <div class="searchbar">
    	<h1>SEARCH</h1>
        	<form class="form-inline" action="SearchEvent.jsp">
   ...
   <div class="timetable-body">
			<div class="timetable-table">
				<table>
	  				<tr class="jumbotron">
	    				<td class="unit-icon"><a href=<%= "\"EventPage.jsp?Id=" + events.get(i).getId() + "\"" %>><img class="unit-image" src="sources/placeholder<%=(int )(Math.random() * 10 + 1)%>.jpg" width="auto" height="180px"/></a></td>
	    				<td style="padding: 10px" class="unit-description">
	    				<h2> <a style="color: #932BA6" href=<%= "\"EventPage.jsp?Id=" + events.get(i).getId() + "\"" %>><%=events.get(i).getName() %> </a></h2>
...
$(function (){
	...
	$.ajax({
		type: 'GET',
		<%if ((from == null || "".equals(from)) && (until == null || "".equals(until))) {%>
			<%if (type == null || "".equals(type)) {%>

		url: 'rest/search/'+<%=searchWords%>,
			<%} else {%>
		url: 'rest/search/'+<%=searchWords%>+'/'+<%=type%>,
			<%}%>

		<%} else if((searchWords == null || "".equals(searchWords)) && (type == null || "".equals(type))) {%>
		url: 'rest/search/'+<%=from%>+'/'+<%=until%>,

		<%} else {%>

		url: 'rest/search/'+<%=from%>+'/'+<%=until%>+'/'+<%=searchWords%>+'/'<%=type%>+'/',

		<%}%>
		success: function(events) {
			$.each(events, function(i, event) {
				$events.append('<li>name'+event.name+'</li>');
			});
		}
	});
